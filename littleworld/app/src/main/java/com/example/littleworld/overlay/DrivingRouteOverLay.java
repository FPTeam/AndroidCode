package com.example.littleworld.overlay;
...
public class DrivingRouteOverLay extends RouteOverlay{
    ...
    public DrivingRouteOverLay(Context context, AMap amap, DrivePath path,
                               LatLonPoint start, LatLonPoint end, List<LatLonPoint> throughPointList, List<Marker> throughMarkerList) {
    	super(context);
    	mContext = context; 
        mAMap = amap; 
        this.drivePath = path;
        ...
        this.throughMarkerList = throughMarkerList;
    }

    public DrivingRouteOverLay(Context context, AMap amap, DrivePath path,
                               LatLonPoint start, LatLonPoint end, List<LatLonPoint> throughPointList, List<Marker> throughMarkerList, Marker sMarker, Marker eMarker) {
        this(context,amap,path, start,end, throughPointList,throughMarkerList);//重载
	...
	public void addToMap() {
        ...
        try {
            ...
            addStartAndEndMarker(sMarker,eMarker);
                ...
                if (latLonPoint != null) {
                    throughPointMarkerList.add(mAMap
                            .addMarker((new MarkerOptions())
                                    ...
                                    .title("景区").snippet(this.throughMarkerList.get(i).getTitle())));
    ...
    private void addThroughPointMarker() {
        if (this.throughMarkerList != null && this.throughMarkerList.size() > 0) {
            ...
            for (int i = 0; i < this.throughMarkerList.size(); i++) {
                marker_new = this.throughMarkerList.get(i);
                if (throughMarkerList != null) {
                    throughPointMarkerList.add(mAMap
                            .addMarker((new MarkerOptions())
                                    .position(
                                            new LatLng(marker_new.getPosition().latitude, marker_new.getPosition().longitude))
                                    .visible(throughPointMarkerVisible)
                                    .icon(getThroughPointBitDes())
                                    .title("\u9014\u7ECF\u70B9").snippet(marker_new.getTitle())));//途经点
                }
            }
        }
    }*/
